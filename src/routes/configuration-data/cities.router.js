...
router.get('/', async function (req, res, next) {
    ...
    try {
        let result = await citiesService.findAndPaginate( params, populate );
...
router.get('/all', async function (req, res, next) {
    ...
    try {
        ...
        let result = await citiesService.findAll( params );
        return ApiResponse(res, 200, true, ResponseMessages.dataFetched, result )
    } catch (error) {
        return next(error)
    }
...
router.get('/count', async function (req, res, next) {
    let params = req.query;
    try {
        let result = await citiesService.getCount( params );
        return ApiResponse(res, 200, true, ResponseMessages.dataFetched, result )
    } catch (error) {
        return next(error)
    }
...
router.get('/:id', async function (req, res, next) {
    ...
    try {
        let result= await citiesService.get( _id );
        return ApiResponse(res, 200, true, ResponseMessages.dataFetched, result )
    } catch (error) {
        return next(error)
    }
...
router.post('/', async function(req, res, next){
    ...
    try {
        let result= await citiesService.createNew( params );
...
router.post('/:id', async function(req, res, next){
    ...
    let params = req.body;
    try {
        let result = await citiesService.updateNew( id, params );
...
router.delete('/:id', async  function(req, res, next){
    let id = req.params.id;
    try {
        let result= await citiesService.delete( id );
