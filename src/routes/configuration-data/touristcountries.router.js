...
const { touristCountriesService } = require("../../services");
...
router.get('/', async function (req, res, next) {
    ...
    try {
        let result = await touristCountriesService.findAndPaginate( params, populate );
...
router.get('/count', async function (req, res, next) {
    let params = req.query;
    try {
        let result = await touristCountriesService.getCount( params );
        return ApiResponse(res, 200, true, ResponseMessages.dataFetched, result )
    } catch (error) {
        return next(error)
    }
...
router.get('/:id', async function (req, res, next) {
    ...
    try {
        let result= await touristCountriesService.get( _id );
        return ApiResponse(res, 200, true, ResponseMessages.dataFetched, result )
    } catch (error) {
        return next(error)
    }
...
router.post('/', async function(req, res, next){
    ...
    try {
        let result= await touristCountriesService.create( params );
...
router.post('/:id', async function(req, res, next){
    ...
    let params = req.body;
    try {
        let result = await touristCountriesService.update( id, params );
...
router.delete('/:id', async  function(req, res, next){
    let id = req.params.id;
    try {
        let result= await touristCountriesService.delete( id );
